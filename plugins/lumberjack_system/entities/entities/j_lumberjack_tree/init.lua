...
function ENT:Initialize()
	...
	self:SetModel("models/props_foliage/urban_tree_giant02_mediuma_kingdom.mdl")
...
if (self.Respawning) then
	local respawnTime = ix.config.Get("TreeRespawn", 60)
...
function ENT:OnTakeDamage(dmg)
	...
	if( player:IsPlayer() and IsValid(player:GetActiveWeapon()) and player:GetActiveWeapon():GetClass() == "weapon_j_lumberjack_axe" ) then
		...
		if (!self.Respawning) and self.health == 0 then
			...
			-- local log = ents.Create('j_lumberjack_log')
			-- log:SetPos(self:GetPos()+Vector(0,0,30))
			-- log:Spawn()
			
			-- local log2 = ents.Create('j_lumberjack_log')
